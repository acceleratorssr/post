...
// - ErrFailedToGetLock: 超过重试次数，但是整个重试过程都没有出现错误
// - DeadlineExceeded 和 ErrFailedToGetLock: 超过重试次数，但是最后一次重试超时了
// 注意：
	...
	for {
		...
		if !ok {
			if err != nil {
				...
				err = fmt.Errorf("锁被人持有: %w", ErrFailedToGetLock)
			}
