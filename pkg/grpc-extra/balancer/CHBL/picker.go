	...
	if b.HashRing == nil {
		...
		for oldOne := range b.subConn {
			...
			if _, exists := currentNodes[oldOne]; !exists {
				...
				var cnt int
				if info.ReadySCs[oldOne].Address.Attributes.Value(grpc_extra.RequestsCount) != nil {
					cnt = info.ReadySCs[oldOne].Address.Attributes.Value(grpc_extra.RequestsCount).(map[string]any)[grpc_extra.RequestsCount].(int)
					avgLoad += cnt
		...
		for newOne := range currentNodes {
			if _, exists := b.subConn[newOne]; !exists {
				...
				var cnt float64
				if info.ReadySCs[newOne].Address.Attributes.Value(grpc_extra.RequestsCount) != nil {
					cnt = info.ReadySCs[newOne].Address.Attributes.Value(grpc_extra.RequestsCount).(float64)
					avgLoad += int(cnt)
					b.HashRing.load[uint32(conPtr)] = cnt
				}
				fmt.Printf("server: %s, load:%f \n", info.ReadySCs[newOne].Address.Addr, cnt)
			}
		}
